...
| `npm run db:seed` | Наповнити демо-даними (повний скид) |
| `npm run db:seed:safe` | Засіяти лише якщо база порожня (для автозасіву на старті) |
| `npm run db:reset` | Скинути БД і пересіяти |

### Оновлення демо-контенту на задеплоєній базі

На старті сервер засіває дані лише якщо база порожня (`db:seed:safe`), тож новий
push контент у проді **не перетирає**. Щоб одноразово оновити демо-дані
(наприклад, після розширення бібліотеки активностей):

1. На Render у сервісі `psycho-program-api` → **Environment** додайте змінну
   `FORCE_RESEED` = `true`.
2. Дочекайтесь авто-деплою (або **Manual Deploy**). На старті база буде
   повністю пересіяна свіжими даними.
3. **Видаліть** `FORCE_RESEED` (або поставте `false`), щоб наступні деплої знову
   не чіпали дані.

> ⚠️ `FORCE_RESEED=true` повністю перезаписує базу демо-даними (видаляє створене
> вручну). Використовуйте лише на демо-середовищі.
